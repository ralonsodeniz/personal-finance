...
3. **Keep a standard OCI image as the later backend/worker boundary.** Deploy the Next.js web/PWA and its Route Handler API together initially. Do not deploy a separate API or always-on worker during the free-tier phase. When bounded asynchronous work appears, use the Supabase Queue/Cron/Edge Function path; keep a normal Dockerfile and platform-neutral `PORT`/health-check contract so a later worker can move to Render, Fly.io, or AWS ECS/Fargate. See [Render monorepo support](https://render.com/docs/monorepo-support), [preview environments](https://render.com/docs/preview-environments), [cron jobs](https://render.com/docs/cronjobs), [environment variables](https://render.com/docs/configure-environment-variables), and [rollbacks](https://render.com/docs/rollbacks).
4. **Managed PostgreSQL is an independent boundary.** Use local development, preview, and production when production is introduced; do not require a persistent staging environment. Keep SQL/ORM migrations in source control, and run a single forward-only migration job from the protected deployment workflow. Do not make the database a Vercel, Cloudflare, or Fly volume dependency. A tool such as Drizzle can implement the migration command, but the application contract should be only a versioned migration directory plus an idempotent `db:migrate` command. PostgreSQL’s [backup and restore documentation](https://www.postgresql.org/docs/current/backup.html) makes regular backups and recovery planning an application responsibility.
5. **Durable jobs use a provider-neutral queue adapter, not request tail work.** During the free-tier phase, bounded asynchronous work uses Supabase Queues (`pgmq`) plus Supabase Cron and short Edge Function consumers. Keep a `JobQueue` interface, idempotency keys, retry counts, and a dead-letter state so the adapter can later move to a paid Render worker with Graphile Worker, Amazon SQS, Cloudflare Queues, or another managed queue. Vercel `after()`/`waitUntil()` and cron are useful only for bounded side effects and schedules, not as the durable job contract.
...
| API | Next.js/Vercel deployment initially; later OCI image if extracted | Vercel project `web` initially | Promote/rollback the known-good deployment |
| Worker | Supabase Edge Function/Cron initially; later OCI image | Not continuously deployed initially; paid Render worker later if required | Replay idempotent jobs or redeploy the previous worker image |
...
3. If `apps/web` or `apps/docs` is affected, create a Vercel preview. If the API contract or backend is affected, create a backend preview only when needed; otherwise test against the shared sanitized preview API. Do not connect previews to production data or production credentials.
4. If migrations changed, apply them to a disposable or dedicated preview database, run compatibility/contract tests, and record the migration plan. A preview migration must never point at production.
...
3. Deploy to the protected `preview` environment when a persistent preview is needed. Run the migration command once, then deploy the web/API preview and bounded queue consumers. Keep migrations expand/contract compatible with both the old and new application version.
...
Use three logical environments, with a deliberate cost/data policy:
...
2. Run SQL/static safety checks in CI and apply it to a disposable or preview database.
...
For the free-tier phase, use Supabase Queues (`pgmq`) as the durable message store and Supabase Cron plus short Edge Function consumers for bounded idempotent batches. Do not deploy an always-on worker yet. If a job exceeds Edge Function runtime/memory limits, requires continuous consumption, or needs higher parallelism, move the same `JobQueue` contract to a paid Render background worker running Graphile Worker, or later to SQS/Cloudflare Queues. Do not select a queue solely because it is next to the frontend host; select it based on delivery semantics, visibility timeout, retry/DLQ operations, region, cost, and exportability.
...
- A later split between Vercel and Render would create more accounts, domains, environment variables, deploy statuses, and incident surfaces than a single-vendor stack.
- Previewing an end-to-end API/database for every PR may be expensive; a shared sanitized preview API is cheaper but less isolated.
...
2. Confirm Vercel and Supabase Free quotas, project pausing, Edge Function/Queue/Cron limits, preview data policy, and the later production plan; do not provision Render until a paid worker is justified.
3. Select the production PostgreSQL region and recovery profile. Record RPO/RTO, backup retention, PITR, encryption, connection pooling, network allow-listing, export procedure, and a recurring restore drill before real financial data.
...
6. Implement and test the free-tier Supabase Queue/Cron/Edge Function path. Document job SLAs, maximum batch duration, retry/deduplication, replay permissions, and dead-letter ownership; define the migration trigger for a paid worker.
...
8. Define preview data and auth policy: database seeding, OAuth callback domains, email delivery suppression, rate limits, and whether the API preview is per PR or shared.
